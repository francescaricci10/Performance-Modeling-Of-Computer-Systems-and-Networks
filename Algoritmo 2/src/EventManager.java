public class EventManager {
    ...
    */
    ...
    /*
        Il metodo restituisce l'indice del server il cui job
        in esecuzione di classe 2 deve essere interrotto
     */
    int findClass2Job(Event[] event) {

        int s;
        int i = 1, k=0;
        Simulator m = new Simulator();
        Event[] event_list = new Event[m.SERVERS + 1];
        Integer[] indexes = new Integer[m.SERVERS + 1];

        // viene restituito l'indice del primo server "attivo"
        // con in esecuzione un job di classe 2
        while (event[i].x == 0 || event[i].type == 1){
            i++;
        }
        s = i;

        /*
        // si cerca tra gli altri server con in esecuzione job di classe 2
        // quello con un evento in cui il tempo di 'next-event' è maggiore
        while (i < m.SERVERS) {
            i++;
            if ((event[i].x == 1) && (event[i].type == 2)
                    && (event[i].t > event[s].t))
                s = i;
        }
        */
